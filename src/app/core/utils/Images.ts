...
export const Images = (() => {
    ...
    const loadImage = async function(imageName: string): Promise<void> {
        try {
            const svg = await fetch(`/moodle-develop/question/type/digitalcircuit/img/items/${imageName}`);
            if (!svg.ok) // Make sure fetch worked
                throw new Error(`Failed to fetch /moodle-develop/question/type/digitalcircuit/img/items/${imageName}: ${svg.statusText}`);
            ...
            if (svgXML.querySelector("parsererror")) { // Make sure there's no XML parsing error
                throw new Error(`Failed to parse XML for /moodle-develop/question/type/digitalcircuit/img/items/${imageName}` +
                                `: ${svgXML.querySelector("parsererror")?.innerHTML}`);
            }

            const drawing = CreateDrawingFromSVG(svgXML, (DEBUG_NO_FILL ? { fillStyle: "none" } : {}));
            if (!drawing)
                throw new Error(`Failed to create drawing for svg: /moodle-develop/question/type/digitalcircuit/img/items/${imageName}`);
